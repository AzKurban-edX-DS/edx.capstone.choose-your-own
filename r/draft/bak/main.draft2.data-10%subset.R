...
%1", ds.0.1train.subset.file_path)
  ...
  x0.9.test <- train.dataset.list$test_set
  y0.9.test <- as.factor(rownames(x0.9.test))
...
# dim_reduction.x0.1.train_knn_pca.file_path <-
#   file.path(knn_pca.path, "dim-reduction.x0.1.train.k1-7.2nn+pca.RData")
 
dim_reduction.x0.1.train_knn_pca.file_path <-
  file.path(knn_pca.path, "dim-reduction.x0.1.train.k4-5-6nn+pca.RData")
...
if (file.exists(dim_reduction.x0.1.train_knn_pca.file_path)) {
  put_log1("Loading Model Fit Data from cache file: 
%1", dim_reduction.x0.1.train_knn_pca.file_path)
  
  load(dim_reduction.x0.1.train_knn_pca.file_path)
  put_log("Train Data list has been loaded from cache.")

} else {
  put_log("Training Model `kNN+PCA` on the dataset subset: `x0.1.train`..." )
  
  train_knn_pca <- caret::train(x0.1.train, y0.1.train, method = "knn", 
...
train_knn_pca.k4.5.6 <- train_knn_pca
  ...
  # save(train_knn_pca.k1_7.2, file = dim_reduction.x0.1.train_knn_pca.file_path)
  save(train_knn_pca.k4.5.6, file = dim_reduction.x0.1.train_knn_pca.file_path)
...
# fine-tuned for sequence of k 1-7 by step 2,
# and tested on %2 size proportion test set:
# %3", 0.1, 0.9, xy0.1.knn_pca.k1_7.2.accuracy)
#> [1] 0.8685502

cl <- makeCluster(N_pcCores)
registerDoParallel(cl)

y0.1_hat_knn_pca.k4.5.6 <- stats::predict(train_knn_pca.k4.5.6, x0.1.test, type = "raw")

xy0.1.knn_pca.k4_6.accuracy <- mean(y0.1_hat_knn_pca.k4.5.6 == y0.1.test)
...
and tested on %2 size proportion test set:
%3", 0.1, 0.9, xy0.1.knn_pca.k4_6.accuracy)
#> [1] 

put_log2("Accuracy of `x0.1` model (trained and tested on subset of %1 size proportion):
%2", 0.1, xy0.1.accuracy)
#> 0.868550221477314

put_log("Predicting on `x0.9.test`")
# start <- put_start_date()
# y_hat_knn_pca.k1_7.2.x0.9.test <- stats::predict(train_knn_pca.k1_7.2, x0.9.test, type = "raw")
# 
# xy0.9.knn_pca.k1_7.2.accuracy <- mean(y_hat_knn_pca.k1_7.2.x0.9.test == y0.9.test)
...
y_hat_knn_pca.k4.5.6.x0.9.test <- stats::predict(train_knn_pca.k4.5.6, x0.9.test, type = "raw")

xy0.9.knn_pca.k4.5.6.accuracy <- mean(y_hat_knn_pca.k4.5.6.x0.9.test == y0.9.test)
xy0.9.knn_pca.k4.5.6.accuracy
#> [1] 0.8410531
...
fine-tuned for sequence of k 4,5,6,
and tested on %2 size proportion test set:
%3", 0.1, 0.9, xy0.9.knn_pca.k4.5.6.accuracy)
#> [1] 

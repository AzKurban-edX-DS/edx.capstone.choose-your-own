...
#   file.path(knn_pca.path, "dim-reduction.x1e3.train.k1-7.2nn+pca.RData")
...
%1", dim_reduction.x1e3.train_knn_pca.file_path)
  ...
  start <- put_start_date()
  # save(train_knn_pca.k1_7.2, file = dim_reduction.x1e3.train_knn_pca.file_path)
  save(train_knn_pca.k4.5.6, file = dim_reduction.x1e3.train_knn_pca.file_path)
  stopCluster(cl)
  stopImplicitCluster()
  put_end_date(start)
  rm(train_knn_pca)
...
cl <- makeCluster(N_pcCores)
registerDoParallel(cl)
# y1e3_hat_knn_pca <- stats::predict(train_knn_pca.k1_7.2, x1e3.test, type = "raw")
y1e3_hat_knn_pca <- stats::predict(train_knn_pca.k4.5.6, x1e3.test, type = "raw")
stopCluster(cl)
stopImplicitCluster()
print_end_date(start)
...
start <- put_start_date()
# y1e3_hat_knn_pca.x.test <- stats::predict(train_knn_pca.k1_7.2, x.test, type = "raw")
y1e3_hat_knn_pca.x.test <- stats::predict(train_knn_pca.k4.5.6, x.test, type = "raw")
print_end_date(start)
# Time difference of 11.01372 mins

mean(y1e3_hat_knn_pca.x.test == y.test)
#> [1] 0.8410531

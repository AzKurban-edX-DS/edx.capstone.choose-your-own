...
dim_reduction.x.train_knn_pca.file_path <- file.path(models.path, "dim-reduction.x.train_knn_pca.RData")

if (file.exists(dim_reduction.x.train_knn_pca.file_path)) {
  put_log1("Loading Model Fit Data from cache file: 
%1", dim_reduction.x.train_knn_pca.file_path)
  
  start <- put_start_date()
  load(dim_reduction.x.train_knn_pca.file_path)
  put_log("Train Data list has been loaded from cache.")
  put_end_date(start)
  ...
  save(train_knn_pca, file = dim_reduction.x.train_knn_pca.file_path)
}

plot(train_knn_pca)
train_knn_pca

put_log("Constructing predictions for the trained model: `kNN(NZV+PCA)+CV`...")
start <- put_start_date()
...
log_close()
#-------------------------------------------------------------------------------
### Open log: Dimension reduction with PCA (probe) -----------------------------
open_logfile(".model.dim-reduction-pca")
#### Dimension reduction with PCA --------------------------------
# Reference:
# Dimension reduction with PCA

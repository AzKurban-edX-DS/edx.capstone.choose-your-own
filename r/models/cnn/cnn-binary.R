...
%3", label.char,
    ...
    {  
      ...
      cnn.bin_model <-   keras_model_sequential(shape(28L, 28L, 1L)) |>
...
%3", label.char, label.idx, capture.output(summary(cnn.bin_model)))
    ...
    {  
      ...
      cnn.1bl.train_history <- cnn.bin_model |> 
        fit(x.train, 
            y.train,
...
%3",label.char, 
  ...
  #### Evaluate Pre-trained Model ************************************    
  {
    put_log("Evaluating the CNN-based Binary  Classifier model for '%1' character...", 
            label)
    eval.result <- cnn.bin_model |> evaluate(x.test, y.test)
    ...
    put_log("Making predictions using the CNN-based Binary classifier model for the character '%1'...", 
            label)
    start <- put_start_date()
    preds <- cnn.bin_model |> predict(x.test) 

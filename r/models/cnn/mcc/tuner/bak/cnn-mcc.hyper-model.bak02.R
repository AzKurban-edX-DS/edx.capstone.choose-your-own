...
CNN_MCC.HyperModel <- reticulate::PyClass(
  ...
  list(
    ...
    `__init__` = function(self, 
      ...
      self$max_conv_blocks = max_conv_blocks
      NULL
    },
    ...
    build = function(self, hp) { # [2]
      ...
      # max_blocks <- ifelse(is.null(self$max_conv_blocks), 5, self$max_conv_blocks)
      
      put_log("Maximum number of Convolution Blocks: %1", self$max_conv_blocks)
      
      conv_blocks <- hp$Int('conv_blocks',
                         min_value = 2,
                         max_value = self$max_conv_blocks,
                         default = 2) |> min(self$max_conv_blocks)

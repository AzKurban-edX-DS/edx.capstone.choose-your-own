...
if(!is.null(cnn_mcc.lr_tuner.result$error) ||
   !is.null(cnn_mcc.lr_tuner.result$hypermodel$error)) {
  put_log("Some error(s) occurred while tuning.")
  
  if(!is.null(cnn_mcc.lr_tuner.result$error))
    put_log(cnn_mcc.lr_tuner.result$error)
  
  if(!is.null(cnn_mcc.lr_tuner.result$hypermodel$error))
    put_log(cnn_mcc.lr_tuner.result$hypermodel$error)
}

cnn_mcc.lr_tuner <- cnn_mcc.lr_tuner.result$tuner
cnn_mcc.tuners$tuned_by.learning_rate <- cnn_mcc.lr_tuner
...
cnn_mcc.lr_tuner$results_summary()
{
  invisible()
}

cnn_mcc.lr_tuner.best_hp <- 
  cnn_mcc.lr_tuner$get_best_hyperparameters(num_trials = 1L)[[1]]

put_log("The best Hyperparameters values:
%1", capture.output(cnn_mcc.lr_tuner.best_hp$values))
{
  # $conv_blocks
  # [1] 2
  # 
...
best_hp.config <- cnn_mcc.lr_tuner.best_hp$get_config()
...
best_trial <- 
  cnn_mcc.lr_tuner$oracle$get_best_trials(num_trials = 1L)[[1]]
...
tcnn_mcc.arch.best_model <- 
  kerastuneR::get_best_models(tuner = cnn_mcc.lr_tuner, 
                              num_models = 1L)[[1]]

tcnn_mcc.arch.best_model |> plot_keras_model(to_file = cnn_mcc.lr_tuner.best_model.plot.img_file,
                                             show_shapes = T)
...
tcnn_mcc.arch.best_hp.model <- 
  cnn_mcc.lr_tuner.result$hypermodel$build(cnn_mcc.lr_tuner.best_hp)
